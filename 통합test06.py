...
TCN_PATH              = 'model_unified_test01.pt' 
YOLO_PATH             = 'poker_tenkey(heavy).pt'   # 키보드 검출 모델
...
# ===== 실시간 상태 (양손을 '독립 시퀀스'로만 관리, 모델은 동일) =====
...
def extract_base_feature(landmarks):
    """
    MediaPipe 21개 (x,y,z) → 손목(0) 기준 상대좌표로 정규화 후 평탄화.
    손 구분/미러링/플래그 없이 '있는 그대로' 특징만 사용.
    """
...
def predict_press(seq_vecs):
    """
    공용 모델로 예측. 출력이 1 또는 2일 수 있어 둘 다 처리.
    """
    if model is None: return None
    arr = np.asarray(seq_vecs, dtype=np.float32)  # (T, C=expected_in_ch=보통 63)
    if arr.shape[1] != model.expected_in_ch:
        # 입력 차원 불일치 시 안전 처리(패딩/자르기)
        if arr.shape[1] < model.expected_in_ch:
    ...
    while True:
        ...
        if res.multi_hand_landmarks:
            for i, lm in enumerate(res.multi_hand_landmarks):
                ...
                # 어느 손인지 MediaPipe가 주는 레이블로 버퍼만 나눠서 저장 (모델 입력은 동일)
            ...
            if curr_act[hand_label] == 'TYPING' and prev_act[hand_label] != 'TYPING' and effect_xy[hand_label]:
                ...
                for k in corrected_boxes:
                    ...
                    if x0 <= x_ <= x0 + w0 and y0 <= y_ <= y0 + h0:
                        ...
                        print(f"[{time.strftime('%H:%M:%S')}] {hand_label} Pressed {k['label']}")
                        break
