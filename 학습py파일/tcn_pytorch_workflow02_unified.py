# -*- coding: utf-8 -*-
"""TCN_pytorch_workflow02_unified.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Ez_ywnwuKN4FforAFqdgaRK7tEb07mXY
"""
...
# ##############################################################################
# @title 셀 2 (통합 모델용): 데이터 로딩 (양손 통합) - 최종 수정 버전
# ##############################################################################
import os
import glob
import pandas as pd
...
# --- 필요한 라이브러리 및 설정 (사용자 코드에 있었으나 누락된 부분 추가) ---
class Config:
    SEQUENCE_LENGTH = 10  # 예시 시퀀스 길이
    TCN_INPUT_CHANNELS = 0
...
try:
    ...
    print("Drive가 이미 마운트되어 있습니다.")


# 2. 데이터 경로 설정
BASE_DATA_DIR = '/content/drive/MyDrive/gesture_data_v3'

# [변경점] 파일 검색 패턴을 더 깊은 하위 폴더까지 모두 검색하도록 수정
# BASE_DATA_DIR 아래의 모든 폴더(**/)에 있는 모든 csv파일(*.csv)을 찾습니다.
FILE_PATTERN = os.path.join(BASE_DATA_DIR, '**', '*.csv')


# 3. 모든 CSV 파일 경로 찾기
# recursive=True 옵션으로 하위 폴더를 모두 탐색합니다.
all_files = glob.glob(FILE_PATTERN, recursive=True)

if not all_files:
    print(f"'{FILE_PATTERN}' 패턴과 일치하는 파일을 찾을 수 없습니다.")
    print("디버깅 결과 폴더 구조는 확인되었으나, 그 안의 CSV 파일을 찾지 못했습니다.")
    print("각 최종 폴더(예: gesture_data_v3_left_김민규_bright) 안에 CSV 파일이 있는지 확인해주세요.")
else:
    # 4. 모든 CSV 파일을 하나로 합치기
    print(f"총 {len(all_files)}개의 CSV 파일을 불러옵니다:")
    # 파일 경로가 너무 길 수 있으니 일부만 출력
    for f in all_files[:5]:
        print(f" - {f}")
    if len(all_files) > 5:
        print(f" - ... (외 {len(all_files) - 5} 개 파일)")
...
# ##############################################################################
# @title 셀 9 (통합 모델용): 🚀 2단계 - T-SMOTE 데이터셋 생성 실행
# ##############################################################################
print("--- 2단계: 통합 T-SMOTE 데이터셋 생성 시작 ---")

# 1. 1단계에서 훈련된 통합 기준 모델을 래퍼 클래스로 불러오기
...
# 5. 생성된 균형 데이터셋을 .npz 파일로 저장 (파일 이름 변경)
